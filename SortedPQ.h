...
public:
    ...
    virtual void push(const TYPE &val) {
        ...
        typename vector<TYPE>::iterator low = lower_bound(data.begin(), data.end(), val, this->compare);
        data.insert(low, val);
        //  for (auto iterator = low; iterator != data.end(); iterator++){
        //     *iterator = *iterator - 1;
        // // for (size_t i = data.size() - 1; i > static_cast<size_t>(index + 1); i--){
